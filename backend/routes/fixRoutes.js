...
if (process.env.NODE_ENV === 'production') {
    router.use([/^\/api\/fix-.*/, /^\/fix-.*/, /^\/api\/debug.*/, /^\/api\/run-.*/, /^\/api\/disparar-.*/], (req, res) => res.status(403).json({ error: 'Rotas de manutenção e diagnóstico desativadas em produção por segurança.' }));
}
