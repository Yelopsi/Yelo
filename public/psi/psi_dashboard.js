...
document.addEventListener('DOMContentLoaded', function() {
        ...
        function renderPage() {
            ...
            pageData.forEach(q => {
                ...
                if (cardBody) {
                    ...
                    qBubble.innerHTML = `<p style="margin:0;">${formatTextContent(questionText)}</p>`;
                    ...
                    if (fallbackContent) fallbackContent.innerHTML = formatTextContent(q.conteudo || q.content || '');
                }
    ...
    // --- LÓGICA DE AVALIAÇÃO DA PLATAFORMA (NPS) ---
    function checkAndShowFeedbackModal() {
        if (!psychologistData || !psychologistData.createdAt) return;
        
        // Verifica se o backend confirmou a avaliação para garantir persistência entre logins/dispositivos
...
async function inicializarForum(preFetchedData = null) {
    ...
    async function loadFullPost(postId) {
        ...
        try {
            ...
            postEl.querySelector('.full-post-content').innerHTML = formatTextContent(post.content);
    ...
    function renderComment(comment, container, prepend = false) {
        ...
        commentEl.querySelector('.comment-body').innerHTML = formatTextContent(comment.content);
        ...
        editForm.querySelector('.save-edit').onclick = async () => {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    bodyEl.innerHTML = formatTextContent(updatedComment.content); // Atualiza UI
                    editForm.remove();
                    bodyEl.style.display = 'block';
    ...
    // Deep link: Abre um post específico se a intenção existir
