    ...
    function populatePage(psi) {
        ...
        if (nameEl && psi.nome) {
            ...
            if (nameParts.length > 1) {
                // XSS MITIGATION: Estrutura HTML estática, dados textuais injetados via textContent
                nameEl.innerHTML = `<span class="first-name-wrapper" style="display: inline-flex; align-items: center; gap: 4px; position: relative;"><span id="safe-first-name"></span><span title="Psicólogo Verificado" class="hero-verified-badge mobile-only-badge" style="display: inline-flex;">${verifiedSvg}</span></span><span id="safe-last-name" class="name-break-mobile"></span><span title="Psicólogo Verificado" class="hero-verified-badge desktop-only-badge" style="display: inline-flex; margin-left: 4px;">${verifiedSvg}</span>`;
                document.getElementById('safe-first-name').textContent = nameParts[0];
                document.getElementById('safe-last-name').textContent = nameParts.slice(1).join(' ');
            } else {
                ...
                document.getElementById('safe-first-name').textContent = psi.nome;
            }
        }
        ...
        setupZapButton('btn-agendar-whatsapp-mobile-reviews');
